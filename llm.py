...
def generate_comment(calc_result: dict, news: list[dict]) -> str:
    from core.scraper import scrape_fak_rates
    from datetime import datetime, timedelta
    import logging
    logger = logging.getLogger(__name__)
    
    logger.info("정형화된 FAK 코멘트 생성을 위해 마켓플레이스 스크래핑을 시작합니다...")
    fak_rates = scrape_fak_rates()
    ...
    # SCFI는 매주 금요일 발표이므로 가장 최근 금요일(과거)로 날짜 설정
    offset = (today.weekday() - 4) % 7
    last_friday = today - timedelta(days=offset)
    prev_friday = last_friday - timedelta(days=7)
    
    dt_str = f"{last_friday.month}/{last_friday.day}"
    prev_dt_str = f"{prev_friday.month}/{prev_friday.day}"
    
    def fmt_pct(d):
        pct = d.get('change_pct', 0)
        return f"+{pct:.0f}%" if pct > 0 else f"{pct:.0f}%"

    chg = comp.get('change', 0)
    pct = comp.get('change_pct', 0)
    chg_sign = "+" if chg > 0 else ""
    pct_sign = "+" if pct > 0 else ""
    
    # 템플릿 텍스트 생성
    comment = f"""○ SCFI : ({dt_str}, {comp.get('current', 0):,.0f} 전주 대비 {chg_sign}{chg:,.0f}, {pct_sign}{pct:.0f}%)
  - {prev_dt_str}, {comp.get('previous', 0):,.0f}pt, '25년 12/31 1,656pt
    · 북미 파나마운하 통행제한, 중국 주요 항만 혼잡에 따른 선복제약 영향 상승
      (북미 동안 {fmt_pct(usec)}, 서안 {fmt_pct(uswc)}, 유럽 {fmt_pct(eu)})

○ '25.4Q 계약 vs. '26.3Q 계약(40FT) vs FAK평균
  - LA=$1,900 : 2,960 : {fak_rates.get('LA', 0):,}
    (전주 8,312)
  - Savannah=$3,160 : 4,370 : {fak_rates.get('Savannah', 0):,}
    (전주 9,886)
  - (독) Hamburg=$1,540 : 3,900 : {fak_rates.get('Hamburg', 0):,}
    (전주 5,061)
  - (호) Sydney (20FT)=$780 : 1,500 : {fak_rates.get('Sydney', 0):,}
    (전주 2,302)"""

    logger.info("코멘트 템플릿 생성 완료")
    return comment
